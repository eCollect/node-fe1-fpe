...
module.exports = class BigNumber {
	...
	add(addative) {
		...
		for (let i = 0; carry !== 0 || i < length; i++) {
			const result = (this._buffer[i] | 0) + (addative[i] | 0) + carry; // eslint-disable-line no-bitwise
	...
	subtract(subtraction) {
		...
		for (let i = 0; carry !== 0 || i < length; i++) {
			const result = ((this._buffer[i] | 0) - (subtraction[i] | 0)) + carry; // eslint-disable-line no-bitwise
			this._buffer[i] = result & 0xFF;  // eslint-disable-line no-bitwise
			carry = result / 255;
		}
